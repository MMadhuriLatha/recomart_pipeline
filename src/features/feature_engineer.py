...
class FeatureEngineer:
    ...
    def create_item_features(self, df: pd.DataFrame, product_df: pd.DataFrame = None) -> pd.DataFrame:
        """
    ...
    def create_similarity_features(self, df: pd.DataFrame, top_k: int = 10) -> pd.DataFrame:
        """
    ...
    def engineer_all_features(self, interactions_df: pd.DataFrame, 
        ...
        with PipelineLogger(self.logger_name) as logger:
            ...
            item_features = self.create_item_features(interactions_df, products_df)
            ...
            interactions_with_features = self.create_temporal_features(interactions_with_features)
            logger.info(f"Temporal features added: {interactions_with_features.shape}")


            # Step 5: Create similarity-based features
            similarity_features = self.create_similarity_features(interactions_df)
